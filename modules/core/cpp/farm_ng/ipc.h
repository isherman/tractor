...
typedef boost::signals2::signal<void(const farm_ng::v1::Event&)>
    EventSignal;
...
class EventBus : public boost::asio::io_service::service {
 public:
  ...
  const std::map<std::string, farm_ng::v1::Event>& GetState()
      const;
...
farm_ng::v1::Resource ArchiveProtobufAsJsonResource(
...
template <typename ProtobufT>
farm_ng::v1::Resource ArchiveProtobufAsBinaryResource(
    const std::string& prefix, const ProtobufT& message) {
...
template <typename T>
farm_ng::v1::Event MakeEvent(std::string name,
                                            const T& message) {
